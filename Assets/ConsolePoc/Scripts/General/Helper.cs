...
#if !UNITY_WEBGL
        public static async Task<(T[] types, U[] objs)> GetObjectTypesFromEnumAsync<T, U>(Func<T, string> strFunction) 
            where T : struct, IConvertible
        {
            T[] types = (T[])System.Enum.GetValues(typeof(T));
            var objects = await Task.WhenAll(types.Select(
                                x => Addressables.LoadAssetAsync<U>(strFunction(x)).Task));
            return (types, objects);
        }

        public static async Task<Dictionary<T, U>> BuildDictionaryTypesFromEnumAsync<T, U>(Func<T, string> strFunction) 
            where T : struct, IConvertible
        {
            var tupples = await GetObjectTypesFromEnumAsync<T, U>(strFunction);
            Dictionary<T, U> dict = new Dictionary<T, U>();
            for (int i = 0; i < tupples.types.Length; i++)
            {
                dict[tupples.types[i]] = tupples.objs[i];
